...
proc mi data=lda4.hearingwide8 seed=808080 simple nimpute=10  MINMAXITer=1000
round=0.1 out=lda4.hearmonotone;
EM CONVERGE = .01;
EM MAXITER = 1000; 
title 'Monotone imputation';
var t0 t3 t6 t9 t12 t15 t18 t21;
mcmc impute = monotone;
run;


/*** Missing data patterns for monotonized***/
proc mi data=lda4.hearmonotone;
ods select misspattern;
var t0 t3 t6 t9 t12 t15 t18 t21;
run;
/*** Transforming data to vertical data ***/
proc sort data=lda4.hearmonotone out=lda4.monosorted;
by imputation id side;
run;
proc transpose data=lda4.monosorted out=lda4.monovertical;
by imputation id side age side1;
run;



/*** MI Sensitivity Analysis with control based pattern imputation ***/
/**** NCMV ****/

proc mi data=lda4.hearmonotone seed=808080 simple out=hear_ncmv nimpute=1;
title 'Model multiple imputation';
class side;
var side age side1 t0 t3 t6 t9 t12 t15 t18 t21;
monotone reg;
mnar model (t0 t3 t6 t9 t12 t15 t18 t21 / modelobs=ncmv);
run;
...
/*** Transforming data to vertical data ***/
proc sort data=hear_ncmv out=hear_ncmvsorted;
by imputation id side;
run;

proc transpose data=hear_ncmvsorted out=hear_ncmvvertical;
by imputation id side age side1;
run;


proc print data=hear_ncmvvertical; run;


data lda4.hear_ncmv_f;
set hear_ncmvvertical; 
time=compress(NAME,'T');
time1=input(time, 8.);
y=col1;
drop col1;
drop name;
...
proc sort data=lda4.hear_ncmv_f;
by imputation;
run;

    /**NCMV Multiple Imputation**/
ods graphics on;
proc mixed data=lda4.hear_ncmv_f method=REML PLOTS(MAXPOINTS=NONE);
class id time side;
by imputation;
model y = age side1 time1 agetime1 side1time1/ covb s;
repeated time / type = sp(exp)(time1) local subject=id r rcorr;
random intercept /  subject=id g s;
random intercept time / type=un subject=side(id) g s vcorr;
ods output SolutionF=ncmvparms CovB=ncmvcovb;
run;
ods graphics off;

/** Combined Analysis **/
proc mianalyze parms=ncmvparms covb(effectvar=rowcol)=ncmvcovb wcov bcov tcov;
modeleffects intercept age side1 time1 agetime1 side1time1;
run;
